/*
 * Frida script to hook CreateFileW and WriteFile to log file operations and text being saved.
 */
...
// Helper to display file access constants
...
Interceptor.attach(Module.getExportByName("kernel32.dll", "WriteFile"), {
    onEnter(args) {
        ...
        // Try UTF-8 first (since 7 bytes suggests UTF-8)
        let text = "";
        try {
            ...
            try {
                // Fallback to UTF-16
                text = buffer.readUtf16String(numBytesToWrite * 2); // Adjust for UTF-16 byte size
            } catch (e2) {
...
// Clean up handles when files are closed
Interceptor.attach(Module.getExportByName("kernel32.dll", "CloseHandle"), {
    onEnter(args) {
        const handle = args[0].toUInt32();
        if (fileHandles.has(handle)) {
            console.log("Closing handle 0x" + handle.toString(16) + " for " + fileHandles.get(handle));
            fileHandles.delete(handle);
        }
    }
});

console.log("Script loaded! Hooking CreateFileW, WriteFile, and CloseHandle...");